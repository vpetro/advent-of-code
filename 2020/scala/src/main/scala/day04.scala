...
object Day04 extends App {
  ...
  val testFilename =
    "/Users/petrov/code/advent-of-code/data/2020/day/4/test_input"
  val fullFilename =
    "/Users/petrov/code/advent-of-code/data/2020/day/4/input"
      ...
      Try(value.toInt).toOption
        .flatMap(intValue =>
          if (intValue >= 2020 && intValue <= 2030)
            Option(new ExpirationYear(intValue) {})
          else None
        )
    }
  }
  ...
  object Height {
    ...
    private def validateCentimeters(value: Int) =
      if (value >= 150 && value <= 193) Some(new Height(value, "cm") {})
      else None

    def fromString(value: String): Option[Height] = {

      parse(value, Parsing.parseHeight(_)) match {
        case Parsed.Success((number, "in"), _) =>
          Try(number.toInt).toOption.flatMap(validateInches)
        case Parsed.Success((number, "cm"), _) =>
          Try(number.toInt).toOption.flatMap(validateCentimeters)
        case _ => None
      }
  ...
  val (batches, lastBatch) =
    File(fullFilename).lineIterator.foldLeft((List[String](), "")) {
      (r, currentLine) =>
        val (batches, currentBatch) = r
        if (currentLine == "") (currentBatch +: batches, "")
        else (batches, currentBatch + " " + currentLine)
    }
